...
def _append_tool_events(tool_events: list[str]) -> None:
...
def handle_player_message(user_input: str) -> None:
    ...
    if settings.enable_streaming:
        tool_events, text_stream, finish = orchestrator.player_turn_stream(
            character=character,
        ...
        def _flush_tools() -> None:
            ...
            if tools_appended or not tool_events:
                return
            _append_tool_events(tool_events)
            tools_appended = True

        with st.chat_message("assistant"):
            full_response = render_streaming_markdown(
                text_stream,
                on_tools_ready=_flush_tools,
            )
        ...
        _append_tool_events(turn.tool_events)
        st.session_state.messages.append(
...
def render_game() -> None:
    ...
    user_input = render_chat_input(disabled=not get_settings().openai_api_key)
    if user_input:
...
def main() -> None:
    ...
    if page == "menu":
        ...
        scenario: Scenario = st.session_state.selected_scenario
        render_character_creation(scenario)
    elif page == "load_save":
