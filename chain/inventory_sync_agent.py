...
class InventorySyncAgent:
    def __init__(self):
        self.llm = create_chat_llm(role="inventory_sync", temperature=0.2)
        system_prompt = (PROMPTS_DIR / "inventory_sync_agent.txt").read_text(encoding="utf-8")
    ...
    def _parse_response(text: str) -> StatePatch:
        ...
        if data is not None:
            try:
                ...
                logger.warning("背包同步补丁 JSON 字段异常: %s", text[:500])
        else:
            logger.warning("背包同步补丁 JSON 解析失败: %s", text[:500] or "（空响应）")
        return StatePatch()
